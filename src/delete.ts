...
if (allServers.error) {
  echo(
    chalk.red("Error getting servers: " + JSON.stringify(allServers["error"]))
  );
  Deno.exit(1);
}
...
if (allServers.data.servers.length > 0) {
  echo(
    chalk.red(
      `🌲 Number of existing Servers: ${JSON.stringify(
  ...
  if (deleteServer.error) {
    echo(
      chalk.red(
        "Error deleting server: " + JSON.stringify(deleteServer["error"])
      )
    );
    Deno.exit(1);
  }
...
if (allVolumes.error) {
  echo(
    chalk.red("Error getting volumes: " + JSON.stringify(allVolumes["error"]))
  );
  Deno.exit(1);
}
...
if (allVolumes.data.volumes.length > 0) {
  echo(
    chalk.red(
      `🌲 Number of existing volumes: ${JSON.stringify(
  ...
  if (deleteVolume.error) {
    echo(
      chalk.red(
        "Error deleting volume: " + JSON.stringify(deleteVolume["error"])
      )
    );
    Deno.exit(1);
  }
